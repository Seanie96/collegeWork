...
out type);
	...
	void ProcDecl(string progName) {
		...
		Expect(9);
		Expect(16);
		while (la.kind == 36 || la.kind == 37 || la.kind == 40) {
	...
	void VarDecl() {
		string name; int type; int n1 = -1; int n2 = -1; int varType; 
		Type(out type);
		Ident(out name);
		varType = 0; 
		if (la.kind == 24) {
	...
	void ConstantDecl() {
		string name; Obj obj; int reg; int type;
		Expect(40);
		Ident(out name);
		obj = tab.NewObj(name, constant, integer, 0, -1, -1); 
		Expect(18);
		Expr(out reg,
out type);
		if (type == integer) {
...
out type);
	...
	void Stat() {
		int type; string name, arrayN; Obj obj; int reg; int n1 = 0, n2 = 0; int offset = 0;
		switch (la.kind) {
		case 2: {
			Ident(out name);
			obj = tab.Find(name); 
...
out type);
					...
					Expect(1);
					srcIndex1 = Convert.ToInt32(t.val); 
					if (la.kind == 25) {
						Get();
						Expect(1);
						srcIndex2 = Convert.ToInt32(t.val); 
					}
					Expect(26);
					Ident(out arrayN);
...
out typeR);
	...
	static readonly bool[,] set = {
		...
		{x,x,T,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,x,x, x,T,x,T, T,T,T,x, x,x,x,x, x,x,x},
